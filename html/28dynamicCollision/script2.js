...
function setup(){
  for(let i = 0; i < numOfBalls; i++ ){
    let ball = {};
    ball.pos = new Vector2d(getRandomNumber(screenW), getRandomNumber(screenH));
    ball.vel = new Vector2d(getRandomNumber(5), getRandomNumber(5));
    ball.point = new Point(ball.pos.dx, ball.pos.dy, Math.floor(getRandomNumber(20)),  getRandomColor);
...
function animate(){
  ...
  for(let i = 0; i < balls.length; i++){
    box(balls[i]);
    balls[i].pos.add(balls[i].vel); // Adds speed.
    balls[i].point.position(balls[i].pos);
    balls[i].point.draw(ctx); // Draws ball.
